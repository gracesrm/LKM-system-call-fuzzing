    ...
    for(i=0; i< NUMOFTOPSYS; i++){
        if(!strcmp(syscall, top5syscall[i])){
            for(j=0; j< NUMOFTOPSYS; j++){
                ...
                if(fl >= MINTOPPER){  
                    thresh = (int)(12*fl/10> 90? 90: 12*fl/10);
                    printk(KERN_INFO "%s thresh to %d\n", syscall, thresh);
                    return true; 
                }
            }
            thresh = THRESHOLD;
...
void add_fd(long fd, const char *filename){
    ...
    char *f2 = "/tmp";
	if(filename[0]== '/'){        
		if ((strncmp(filename, f, 5)!=0)&&(strncmp(filename, f2, 4)!=0)){ //not under /home, key fd 
			fdlist[fd]= 1;
			printk(KERN_INFO "fd %ld inserted with file name %s\n", fd, filename);
		}
        else{
            if(filename[13]=='.'){
                fdlist[fd]= 1;
...
bool isKeyfile(const char *filename){
    if(filename[0]== '/'){
        if ((strncmp("/home", filename, 5)!=0)&&(strncmp("/tmp", filename, 4)!=0)){ //not under /home, key fd 
            return true;
        }
        if(filename[13]=='.'){
            return true;
